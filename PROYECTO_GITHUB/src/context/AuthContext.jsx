...
// Catálogo de usuarios por defecto del sistema
export const DEFAULT_USERS = [
    ...
    },
    {
        ...
        username: 'roger',
        password: 'admin123',
        nombre_completo: 'Rogelio Aponte',
        nombre_vendedor: 'Rogelio Aponte',
        ...
        permisos: ['pos', 'dashboard', 'inventory', 'cuentas-por-cobrar', 'cuentas-por-pagar', 'reportes', 'configuracion'],
        estado: 'activo'
    },
    {
        id: 'usr_1790276654660',
        username: 'caja1',
        password: '1234',
        nombre_completo: 'Ana Rodriguez',
        nombre_vendedor: 'Ana Rodriguez',
        cargo: 'Cajero de Turno',
        rol: 'cajero',
        foto_url: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=400&auto=format&fit=crop&q=80',
        permisos: ['pos', 'cuentas-por-cobrar'],
        estado: 'activo'
    },
    {
        id: 'usr_1790276843396',
        username: 'vendedor1',
        password: '1234',
        nombre_completo: 'Edwin Gómez',
        nombre_vendedor: 'Edwin Gómez',
        cargo: 'Cajero de Turno',
        rol: 'vendedor',
...
export const DEFAULT_MASTER_ADMIN = DEFAULT_USERS[0]
...
export const AuthProvider = ({ children }) => {
    ...
    const [usuarios, setUsuarios] = useState(() => {
        try {
            ...
            return (list && list.length > 0) ? list : DEFAULT_USERS
        } catch (e) {
    ...
    const refreshUsuarios = useCallback(async () => {
        ...
        try {
            // 1. Verificar si hay usuarios en cache
            let list = gsService.getUsuarios()
            if (list && list.length > 1) {
                setUsuarios(list)
            }

            // 2. Si la cache no está completa o solo tiene 1 usuario, consultar Google Sheets
            if (!gsService.cache?.Usuarios || gsService.cache.Usuarios.length <= 1) {
                await gsService.initialize()
                list = gsService.getUsuarios()
            }
    ...
    // Escuchar cuando el servicio de Google Sheets inicialice datos frescos en segundo plano
    useEffect(() => {
        ...
        window.addEventListener('mme_data_initialized', handleDataInit)
        return () => window.removeEventListener('mme_data_initialized', handleDataInit)
    }, [])
