...
const ProfileSelection = () => {
    const { login, usuarios, refreshUsuarios, loadingUsuarios } = useAuth()
    const [selectedUser, setSelectedUser] = useState(null)
    const [passwordInput, setPasswordInput] = useState('')
    const [showPassword, setShowPassword] = useState(false)
    const [errorMsg, setErrorMsg] = useState('')

    // Cargar y sincronizar perfiles frescos desde Google Sheets
    useEffect(() => {
        if (refreshUsuarios) refreshUsuarios()
    }, [refreshUsuarios])

    // Lista de usuarios ordenados por jerarquía de mayor a menor
    const displayUsers = useMemo(() => {
        const raw = (usuarios && usuarios.length > 0) ? usuarios : (DEFAULT_USERS || [])
